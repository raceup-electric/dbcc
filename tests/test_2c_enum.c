...
int test_2c_enum(void) {
	...
	encode_0x003_ivt_ctrl_fuse_state(&ivtctrl, CAN_0X003_IVTCTRL_IVT_CTRL_FUSE_STATE_FUSE_AND_FUSE_CLOSED);
	...
	encode_0x004_state(&enum2, 7u);
	decode_0x004_state(&enum2, &enum2_out);
	CHECK(enum2_out == 7u);

	encode_0x122_ivt_sleep_ack(&ack, CAN_0X122_IVTSLEEPACK_IVT_SLEEP_ACK_SLEEP_REQUEST_ACKNOWLEDGED);

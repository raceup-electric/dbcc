/* Generated by DBCC, see <https://github.com/howerj/dbcc>

This file was generated by dbcc: See <https://github.com/howerj/dbcc>

For the professional version of this utility visit https://subleq.co.uk
The professional version includes support for CAN-FD, more languages, and
better MISRA-C support.


*/

#include "enum.h"
#include <inttypes.h>
#include <string.h>
#include <assert.h>

static inline uint64_t reverse_byte_order(uint64_t x) {
	x = (x & 0x00000000FFFFFFFF) << 32 | (x & 0xFFFFFFFF00000000) >> 32;
	x = (x & 0x0000FFFF0000FFFF) << 16 | (x & 0xFFFF0000FFFF0000) >> 16;
	x = (x & 0x00FF00FF00FF00FF) << 8  | (x & 0xFF00FF00FF00FF00) >> 8;
	return x;
}

void decode_can_0x001_state(can_0x001_Enum1_obj_t *o, can_0x001_Enum1_state_e *out) {
	assert(o);
	assert(out);
	uint64_t _payload_sig = (uint64_t)(o->payload);
	uint64_t _lane_sig = _payload_sig;
	uint64_t x = _lane_sig & 0xfuLL;
	can_0x001_Enum1_state_e rval = (can_0x001_Enum1_state_e)x;
	*out = rval;
	return;
}

void encode_can_0x001_state(can_0x001_Enum1_obj_t *o, can_0x001_Enum1_state_e in) {
	assert(o);
	uint64_t x = ((uint64_t)in) & 0xfuLL;
	o->payload = (((uint64_t)o->payload) & ~0xfuLL) | (x & 0xfuLL);
	return;
}

void decode_can_0x003_ivt_ctrl_fuse_state(can_0x003_IVTCtrl_obj_t *o, can_0x003_IVTCtrl_ivt_ctrl_fuse_state_e *out) {
	assert(o);
	assert(out);
	uint64_t _payload_sig = (uint64_t)(o->payload);
	uint64_t _lane_sig = _payload_sig;
	uint64_t x = (_lane_sig >> 13) & 0x1uLL;
	can_0x003_IVTCtrl_ivt_ctrl_fuse_state_e rval = (can_0x003_IVTCtrl_ivt_ctrl_fuse_state_e)x;
	*out = rval;
	return;
}

void encode_can_0x003_ivt_ctrl_fuse_state(can_0x003_IVTCtrl_obj_t *o, can_0x003_IVTCtrl_ivt_ctrl_fuse_state_e in) {
	assert(o);
	uint64_t x = ((uint64_t)in) & 0x1uLL;
	o->payload = (((uint64_t)o->payload) & ~(0x1uLL << 13)) | ((x & 0x1uLL) << 13);
	return;
}

void decode_can_0x004_state(can_0x004_Enum2_obj_t *o, uint8_t *out) {
	assert(o);
	assert(out);
	uint64_t _payload_sig = (uint64_t)(o->payload);
	uint64_t _lane_sig = _payload_sig;
	uint64_t x = _lane_sig & 0xfuLL;
	uint8_t rval = (uint8_t)x;
	*out = rval;
	return;
}

void encode_can_0x004_state(can_0x004_Enum2_obj_t *o, uint8_t in) {
	assert(o);
	uint64_t x = ((uint64_t)in) & 0xfuLL;
	o->payload = (((uint64_t)o->payload) & ~0xfuLL) | (x & 0xfuLL);
	return;
}

void decode_can_0x122_ivt_sleep_ack(can_0x122_IVTSleepAck_obj_t *o, can_0x122_IVTSleepAck_ivt_sleep_ack_e *out) {
	assert(o);
	assert(out);
	uint64_t _payload_sig = (uint64_t)(o->payload);
	uint64_t _lane_sig = _payload_sig;
	uint64_t x = _lane_sig & 0xfuLL;
	if ((x & 0x8uLL) != 0uLL)
		x |= ~0xfuLL;
	can_0x122_IVTSleepAck_ivt_sleep_ack_e rval = (can_0x122_IVTSleepAck_ivt_sleep_ack_e)((int64_t)x);
	*out = rval;
	return;
}

void encode_can_0x122_ivt_sleep_ack(can_0x122_IVTSleepAck_obj_t *o, can_0x122_IVTSleepAck_ivt_sleep_ack_e in) {
	assert(o);
	uint64_t x = ((uint64_t)((int64_t)in)) & 0xfuLL;
	o->payload = (((uint64_t)o->payload) & ~0xfuLL) | (x & 0xfuLL);
	return;
}


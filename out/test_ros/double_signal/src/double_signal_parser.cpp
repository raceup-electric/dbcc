...
class DoubleSignalParser : public rclcpp::Node {
public:
	DoubleSignalParser() : Node("double_signal_parser", "/double_signal") {
		createPublishers();
		createSubscriptions();
	}
...
private:
	void createSubscriptions() {
		...
		new_message0_sub_ = this->create_subscription<double_signal::msg::NewMessage0>(
			"new_message0", qos, [this](const double_signal::msg::NewMessage0::SharedPtr msg) {
				if (msg->header.received) return;
				uint64_t x;
				uint64_t i = 0;
				/* double_signal_0: start-bit 0, length 64, endianess intel, scaling 1, offset 0 */
				x = (pack754_64((msg->double_signal_0))) & 0xffffffffffffffff;
				i |= x;
				uint64_t data = (i);
		...
		frame_subscription_ = this->create_subscription<can_msgs::msg::Frame>(
			"/can/double_signal/read", qos, [this](const can_msgs::msg::Frame::SharedPtr msg) {
	...
	void decodeMessage(uint64_t data, uint8_t dlc, uint32_t id, const rclcpp::Time& timestamp) {
		switch(id) {
			case 1024: { /* 0x400: NewMessage0 */
				if (dlc < 8) return;
				double_signal::msg::NewMessage0 msg;
				...
				msg.double_signal_0 = unpack754_64(x);
	...
	rclcpp::Subscription<double_signal::msg::NewMessage0>::SharedPtr new_message0_sub_;
	rclcpp::Publisher<double_signal::msg::NewMessage0>::SharedPtr new_message0_pub_;
...
int main(int argc, char **argv) {
	...
	auto node = std::make_shared<DoubleSignalParser>();
